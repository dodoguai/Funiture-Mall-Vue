   ...
   <div class="box-card">
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        ...
        <span style="margin-left: 10px" v-if='scope.row.level==0'>一级分类</span>
        <span style="margin-left: 10px" v-if='scope.row.level==1'>二级分类</span>
      </template>
    </el-table-column>
    <el-table-column
      label="商品数量"
   ...
   methods: {
      ...
      handleEdit(id){
         ...
         this.$axios({
            ...
            Object.keys(_this.form).forEach(key=>{_this.form[key]=_this.productinfo[key]})
         })
      },
      changetag(num){
         let _this=this;
      ...
      onsubmit(formName){
         this.$refs[formName].validate((valid) => {
            if (valid) {
            let param={'name':this.form.name,'icon':this.form.icon,'keywords':this.form.keywords,'parentId':this.form.parentId,'navStatus':this.form.navStatus,'level':0,'productCount':0, 'sort':this.form.sort,'id':''}
            console.log(param)
         this.$axios({
          method:'post',
...
this.$message({
      ...
      onupdate(formName){
         this.$refs[formName].validate((valid) => {
            if (valid) {
            let param={'name':this.form.name,'icon':this.form.icon,'keywords':this.form.keywords,'parentId':this.form.parentId,'navStatus':this.form.navStatus,'level':0,'productCount':0, 'sort':this.form.sort,'id':this.form.id}
            console.log(param)
         this.$axios({
          method:'post',
          ...
          this.getproductCategory(0,10);}else{
this.$message({
             type:'info',
             message:res.data.message,
          })
